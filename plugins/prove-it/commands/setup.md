...
- `GATE_DIR = ~/.claude/prove-it`. This is where the gate keeps its state: `prove-it-gate` hardcodes it, and `gate-check.sh` defaults to it (honoring a `PROVE_IT_GATE_DIR` override used only for testing). Install to this directory; anywhere else would leave the scripts unable to find their own state.
